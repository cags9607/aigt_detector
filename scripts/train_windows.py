...
- Also produces a zip: <output_dir>/best.zip with:
    best/ (folder) + history.json at the zip root (for convenience, and to follow same format as in the huggingface repo)

Usage example:

python scripts/train_windows.py \
  --data-csv data/qlora_training_windows_FR.csv \
...
def compute_full_metrics(y_true: np.ndarray, y_pred: np.ndarray, threshold: float = 0.5) -> Dict[str, float]:
    ...
    # classification derived from teacher (Pangram Detector) at threshold
...
def main():
    ...
    full_df = load_windows_csv(
        ...
        group_col=cfg.group_col, # ID used to identify a window with the article it came from. Must be unique.
    )
    ...
    # This loss function is not the same used by Pangram Detector (as stated in their arxiv paper); instead, we use a loss function that is more suited for model distillation.
    ...
    # Training data is scarce, so we use other sources to estimate performance metrics, instead of using a validation data set. Anyways, hyperparameters are selected as stated in the EditLens paper, so they've already done the validation step for us ;)
    # -------------------------
